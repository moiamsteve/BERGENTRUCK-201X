...
@export var random_scaling_min: float = 0.75
@export var random_scaling_max: float = 1.25

func _ready() -> void :
	rotation_degrees = randi_range(0, 360)
	var rand_scale = randf_range(random_scaling_min, random_scaling_max)
...
func use() -> void :
	var destroyanim = preload("uid://b58co3jtahjlj").instantiate()
